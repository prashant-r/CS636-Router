...
#if UIP_CONF_ROUTER
  printf("Creating link local all router mcast \n");
  uip_create_linklocal_allrouters_mcast(&loc_fipaddr);
  uip_ds6_maddr_add(&loc_fipaddr);
  if(uip_ds6_maddr_lookup(&loc_fipaddr) == NULL)
  {
    printf("link local all router mcast not added\n");
  }
  else
  {
    printf("link local all router added \n");
  }
// #if UIP_ND6_SEND_RA
...
uip_ds6_addr_add(uip_ipaddr_t *ipaddr, unsigned long vlifetime, uint8_t type)
{
  if(uip_ds6_list_loop
    ...
    PRINTF("Adding this new address \n");
 ...
 */
uip_ds6_addr_t *
uip_ds6_get_link_local(char state, char type)
{
  PRINTF("Came here ");
  for(locaddr = uip_ds6_if.addr_list;
      locaddr < uip_ds6_if.addr_list + UIP_DS6_ADDR_NB; locaddr++) {
    PRINTF("Now what ? ");
    PRINTF("%d , ", locaddr->isused);
    PRINTF("%d , ",locaddr->state);
    PRINTF("%d, ", locaddr->type == type);
    PRINTF("%d , ", uip_is_addr_linklocal(&locaddr->ipaddr));
    PRINT6ADDR(&locaddr->ipaddr);
    PRINTF("\n");
    if(locaddr->isused && (state == -1 || locaddr->state == state) && (type == -1 || locaddr->type == type)
...
uip_ds6_select_src(uip_ipaddr_t *src, uip_ipaddr_t *dst)
{
  ...
  if(!uip_is_addr_linklocal(dst) && !uip_is_addr_mcast(dst)) {
    ...
    matchaddr = uip_ds6_get_link_local(ADDR_PREFERRED, ADDR_AUTOCONF);
  }
  matchaddr = uip_ds6_get_link_local(ADDR_PREFERRED, ADDR_AUTOCONF);
...
uip_ds6_send_ra_periodic(void)
{
  ...
  rand_time = UIP_ND6_MIN_RA_INTERVAL + rand() %
